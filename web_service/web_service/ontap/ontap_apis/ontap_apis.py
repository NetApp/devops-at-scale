...
class Volume(object):
    ...
    def get_snapdiff(self, base, previous):
        ...
        url = self.aggregate.api_server.get_url("ontap/snapshots/{}/files?base_snapshot_key={}".format(base_snapshot_key, previous_snapshot_key))
        headers = self.aggregate.api_server.get_headers()
    ...
    def make_clone(self, snapshot_name, clone_name, uid, gid):
        ...
        users = {'uid': uid, 'gid': gid} if (uid != "" and gid != "") \
                else self.get_uid_gid()
    ...
    def delete_all_clones(self):
        ...
        if clones:
            ...
            for clone_name in clone_names:
                ...
                if status != "COMPLETED":
                    logging.error("Failed to Delete all clones: Status: %s, Error: %s, Error 2: %s", status, error_message, error_message2)
...
class APIServer(object):
    ...
    def get_url(self, url, version="2.0"):
...
def main():
        ...
        if args.create_volume:
            ...
            for vol in args.vol_names.split(','):
                ...
                matchSize = re.match(r'^(\d*)\s?([m|M][i|I]?[B|b]?)?$', args.vol_size)
