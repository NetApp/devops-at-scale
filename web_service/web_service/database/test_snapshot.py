...
class TestSnapshot(unittest.TestCase):
    ...
    def test_purge_db_inconsistency(self, mock_get_snapshot_list,
                                    mock_get_snapshots, mock_db_config, mock_connect_db):
        """Test deletion of inconsistent snapshots from DB"""
        mock_db_config.return_value = {
            'ontap_api': 'a', 'ontap_apiuser': 'b', 'ontap_apipass': 'c',
            'ontap_svm_name': 'd', 'ontap_aggr_name': 'e', 'ontap_data_ip': 'f'
        }
        mock_get_snapshots.return_value = [Mock(value='delete_me'), Mock(value='test_1')]
        mock_get_snapshot_list.return_value = [{"snapshot_name": "test_1", "timestamp": 1500768910}]
    ...
    @patch('web_service.ontap.ontap_service.OntapService.get_snapshot_list')
    def test_purge_snapshots_by_volume(self, mock_get_snapshot_list, mock_helper, mock_db_config,
                                       mock_connect_db, mock_del_snapshot):
        ...
        mock_get_snapshot_list.return_value = [
            ...
            {"snapshot_name": "test_2", "timestamp": 200}]
